...
public class MIBadgeButton: UIButton {
    ...
    private func setupBadgeViewWithString(badgeText: String?) {
        ...
        if badgeLabel.text == "" && MIBadgeButton.lastValue != nil{
            badgeLabel.text = MIBadgeButton.lastValue
        }
        if badgeText != ""{
            MIBadgeButton.lastValue = badgeText
        }
        ...
        if let badgeInset = self.badgeEdgeInsets {
            ...
            let x = self.frame.width - CGFloat((width / 2.0)) - 5
            let y = CGFloat(-(height / 2.0))+7
