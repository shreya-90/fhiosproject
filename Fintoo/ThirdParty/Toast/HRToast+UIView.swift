...
var HRProgressView: UnsafePointer<UIView>?    =   nil

var HRToastProgressValue: UnsafePointer<Float>?        =   nil
...
extension UIView {
    ...
    *  public methods
    */
    class func hr_setToastThemeColor(_ color: UIColor) {
    ...
    class func hr_setToastTitleFontName(_ fontName: String) {
    ...
    class func hr_toastTitleFontName() -> String {
        ...
        if name == nil {
            ...
            name = font.fontName
            UIView.hr_setToastTitleFontName(name!)
        }
        
        return name!
    }
    
    class func hr_setToastFontName(_ fontName: String) {
    ...
    class func hr_toastFontName() -> String {
        ...
        if name == nil {
            let font = UIFont.systemFont(ofSize: 12.0)
            name = font.fontName
            UIView.hr_setToastFontName(name!)
        }
        
        return name!
    }
    
    class func hr_setToastFontColor(_ color: UIColor) {
    ...
    func showToast(_ toast: UIView) {
        showToast(toast, duration: HRToastDefaultDuration, position: HRToastPositionDefault as AnyObject)
    }
    
    fileprivate func showToast(_ toast: UIView, duration: Double, position: AnyObject)
    {
        ...
        if existToast != nil {
            ...
            hideToast(existToast!, force: false);
            print("hide exist!")
        }
    ...
    fileprivate func makeToastActivity(position pos: AnyObject, message msg: String = "")
    {
        ...
        let mainView = UIView(frame: appDelegate.window!.bounds )
//        appDelegate.window!.addSubview(mainView)
//        var mainView = UIView(frame: UIScreen.mainScreen().bounds )
        ...
        objc_setAssociatedObject(self, &HRToastActivityView, mainView, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        
        UIView.animate(withDuration: HRToastFadeDuration, delay: 0.0, options: .curveEaseOut, animations: {
    ...
    */
    ...
    @objc func toastTimerDidFinish(_ timer: Timer) {
        hideToast(timer.userInfo as! UIView)
    }
